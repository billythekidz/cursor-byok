...
function activeAdaptersOf(group) {
  return group?.adapters.filter((adapter) => adapter.active) || [];
}
...
async function handleToggleActive(adapter) {
  ...
  for (const item of current) {
    const sameAdapter = adapter.id && item.id
      ? item.id === adapter.id
      : item.type === adapter.type &&
        item.modelID === adapter.modelID &&
        item.baseURL === adapter.baseURL &&
        item.apiKey === adapter.apiKey;
    if (sameAdapter) {
        ...
        <div v-else>
          ...
          >
            ...
            <div class="mb-4">
              <div class="mb-2 flex items-end justify-between gap-3">
                <div>
                  <div class="text-sm font-medium text-white">已启用模型</div>
                  <div class="mt-0.5 text-xs text-[#737373]">Cursor 可以看到并使用这些模型。</div>
                </div>
                <span class="rounded-full border border-[#1ca35a] bg-[#123322] px-2 py-1 text-[11px] text-[#10AD5D]">
                  {{ activeAdaptersOf(expandedGroup).length }} 个模型
                </span>
              </div>
              <div
                v-if="activeAdaptersOf(expandedGroup).length === 0"
            ...
            <div>
              <div class="mb-2 flex items-end justify-between gap-3">
                <div>
                  <div class="text-sm font-medium text-white">可用模型</div>
                  <div class="mt-0.5 text-xs text-[#737373]">已配置但未启用，启用后 Cursor 才能看到。</div>
                </div>
                <span class="rounded-full border border-[#3f3f3f] px-2 py-1 text-[11px] text-[#8f8f8f]">
                  {{ expandedGroup.adapters.length - activeAdaptersOf(expandedGroup).length }} 个模型
                </span>
              </div>
              <div
                v-if="activeAdaptersOf(expandedGroup).length === expandedGroup.adapters.length"
                class="flex h-[84px] items-center justify-center rounded-[8px] border border-dashed border-[#3a3a3a] bg-[#1f1f1f] px-4 text-center text-xs text-[#737373]"
              >
                该 endpoint 的所有模型都已启用。
              </div>
